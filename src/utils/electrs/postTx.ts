...
export const postTx = (tx: string): Promise<Result<string, APIError<'INTERNAL_SERVER_ERROR'>>> =>
  new Promise((resolve) => {
    fetch(`${BLOCKEXPLORERURL}/tx`, {
      ...
      .then(async (response) => {
        if (response.status !== 200) return resolve(getError({ error: 'INTERNAL_SERVER_ERROR' }))
