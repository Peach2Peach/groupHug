...
describe('zrangewithscores', () => {
  beforeEach(async () => {
    await db.transaction(async (client) => {
      ...
      await client.zadd('test-set-key', 30, 'c')
      await client.zadd('test-set-key', 40, 'd')
    })
  })
  ...
  it('should return items with scores in reverse', async () => {
    const entry = await db.zrangewithscores('test-set-key', undefined, undefined, true, true)
    expect(entry).to.deep.equal([
      {
        score: 40,
        value: 'd',
